...
.msg-row.mine .msg-bubble{background:rgba(59,158,255,.1);border:1px solid rgba(59,158,255,.2);border-bottom-right-radius:3px;color:var(--text);}
...
.chat-input-wrap{flex:1;background:var(--surface2);border:1px solid var(--border);border-radius:10px;display:flex;align-items:flex-end;transition:border-color .15s;}
.chat-input-wrap:focus-within{border-color:var(--blue);box-shadow:0 0 0 2px rgba(59,158,255,.08);}
...
.chat-send-btn{width:38px;height:38px;border-radius:9px;background:var(--blue);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .12s;flex-shrink:0;box-shadow:0 2px 8px rgba(59,158,255,.25);}
.chat-send-btn:hover{background:#2288ee;transform:translateY(-1px);}
...
<div class="comms-layout">
    <div class="comms-sidebar">
        ...
        <div class="contact-list">
            <a href="{{ route('responder.comms') }}" class="group-item {{ !$withUser ? 'active' : '' }}">
            ...
            <a href="{{ route('responder.comms') }}?with={{ $conv['user']->id }}" class="contact-item {{ $withUser?->id == $conv['user']->id ? 'active' : '' }}">
            ...
            @endforeach
            @php $convUserIds = collect($conversations)->pluck('user.id')->toArray(); $newUsers = $users->filter(fn($u) => !in_array($u->id, $convUserIds)); @endphp
            @if($newUsers->count() > 0)
            ...
            <a href="{{ route('responder.comms') }}?with={{ $u->id }}" class="contact-item {{ $withUser?->id == $u->id ? 'active' : '' }}">
    ...
    <div class="comms-chat">
        <div class="chat-header">
            ...
            <div>
                ...
                <div class="ch-role">{{ $withUser->getRoleLabel() }}</div>
            </div>
            @else
...
@push('scripts')
<script>
const SEND_URL  = "{{ route('responder.comms.send') }}";
const FETCH_URL = "{{ route('responder.comms.fetch') }}";
...
function buildMsgHTML(m){
    ...
    return `<div class="msg-row ${mine?'mine':''}" data-id="${m.id}">
        ...
        <div class="msg-bubble-wrap">
            <div class="msg-meta"><span>${m.sender_name}</span>${recv}<span class="msg-channel ${m.channel}">${m.channel}</span><span>${m.time}</span></div>
...
async function sendMessage(){
    ...
    try{
        ...
        appendMessage(data); scrollToBottom();
...
async function poll(){
    try{
        ...
        let added=0;
        msgs.forEach(m=>{ if(!document.querySelector(`[data-id="${m.id}"]`)){appendMessage(m);added++;} if(m.id>lastMsgId) lastMsgId=m.id; });
