...
<div class="page-header">
    <div><h1>Communications</h1><div class="breadcrumb">Responder / Comms</div></div>
</div>
...
<div class="grid-2" style="align-items:start">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('responder.comms.send') }}">
                @csrf
                <div class="form-group">
                    ...
                    <select name="receiver_id" class="form-control">
                        ...
                        @foreach($users as $u)
                        <option value="{{ $u->id }}">{{ $u->name }} ({{ $u->getRoleLabel() }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
    ...
    <div class="card">
        ...
        <div style="max-height:460px;overflow-y:auto;padding:16px">
            @forelse($messages as $msg)
            <div style="margin-bottom:14px;display:flex;gap:10px;{{ $msg->sender_id === auth()->id() ? 'flex-direction:row-reverse' : '' }}">
                <div style="width:30px;height:30px;border-radius:50%;background:var(--surface2);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:.72rem;font-weight:700">
                ...
                <div style="max-width:80%;{{ $msg->sender_id === auth()->id() ? 'text-align:right' : '' }}">
                    <div style="font-size:.7rem;color:var(--text-muted);margin-bottom:3px">
                        {{ $msg->sender->name }}
                        @if($msg->receiver)→ {{ $msg->receiver->name }}@else→ Broadcast@endif
                        · <span class="badge badge-info" style="font-size:.62rem">{{ $msg->channel }}</span>
                        · {{ $msg->created_at->diffForHumans() }}
                    </div>
                    <div style="background:{{ $msg->sender_id === auth()->id() ? 'rgba(56,139,253,.1)' : 'var(--surface2)' }};border:1px solid {{ $msg->sender_id === auth()->id() ? 'rgba(56,139,253,.25)' : 'var(--border)' }};border-radius:8px;padding:9px 12px;font-size:.82rem;line-height:1.5">
                        {{ $msg->content }}
                    </div>
                </div>
            </div>
