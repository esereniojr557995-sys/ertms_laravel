...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sb-user .uname { font-weight: 600; font-size: .84rem; color: var(--text-bright); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; letter-spacing: -.01em; }
        .sb-user .urole { font-size: .68rem; color: var(--text-muted); margin-top: 2px; text-transform: uppercase; letter-spacing: .08em; font-weight: 500; }
        .sb-user .ustatus { display: flex; align-items: center; gap: 5px; margin-top: 7px; }
        .ustatus-dot { width: 5px; height: 5px; background: var(--green); border-radius: 50%; box-shadow: 0 0 6px var(--green); animation: pulse-dot 2.5s ease-in-out infinite; }
        .sb-user .ustatus span { font-size: .66rem; color: var(--green); font-weight: 600; letter-spacing: .06em; text-transform: uppercase; }
        @keyframes pulse-dot { 0%, 100% { opacity: 1; } 50% { opacity: .4; } }

        .sb-nav { flex: 1; overflow-y: auto; padding: 4px 0 8px; scrollbar-width: thin; scrollbar-color: var(--border2) transparent; }
        .sb-nav::-webkit-scrollbar { width: 3px; }
        .sb-nav::-webkit-scrollbar-thumb { background: var(--border2); border-radius: 2px; }

        .sb-section { padding: 14px 16px 5px; font-size: .62rem; font-weight: 600; color: var(--text-dim); text-transform: uppercase; letter-spacing: .16em; }

        .sb-nav a {
            display: flex; align-items: center; gap: 8px;
            padding: 8px 11px 8px 14px; margin: 1px 8px;
        ...
        .sb-nav a svg { width: 15px; height: 15px; flex-shrink: 0; stroke-width: 2; }
        .sb-nav a:hover { background: var(--surface2); color: var(--text); }
        .sb-nav a.active { background: var(--accent-dim); color: var(--accent2); border: 1px solid rgba(240,73,35,.12); }
        .sb-nav a.active svg { color: var(--accent); }
        .sb-nav a .nb { margin-left: auto; background: var(--red); color: #fff; font-size: .62rem; font-weight: 700; padding: 1px 5px; border-radius: 10px; min-width: 16px; text-align: center; font-family: var(--font-mono); }

        .sb-footer { padding: 8px 10px 10px; border-top: 1px solid var(--border); flex-shrink: 0; }
        .sb-footer form button {
            display: flex; align-items: center; gap: 7px; width: 100%;
            padding: 8px 11px; border-radius: var(--r-sm);
            background: transparent; border: 1px solid var(--border);
            color: var(--text-muted); font-size: .82rem;
            font-family: var(--font); cursor: pointer; transition: all .12s; letter-spacing: -.01em;
        }
        .sb-footer form button svg { width: 14px; height: 14px; }
        .sb-footer form button:hover { background: var(--red-dim); color: var(--red); border-color: rgba(232,69,69,.2); }
        ...
        .topbar-left .page-label { font-size: .95rem; font-weight: 600; color: var(--text-bright); letter-spacing: -.02em; }
        .topbar-right { display: flex; align-items: center; gap: 12px; }
        .topbar-status {
            display: flex; align-items: center; gap: 5px;
            font-size: .72rem; font-weight: 600; color: var(--green);
            background: var(--green-dim); border: 1px solid rgba(30,201,109,.12);
            padding: 4px 10px; border-radius: 20px; letter-spacing: .04em; text-transform: uppercase;
        }
        .topbar-status .pulse { width: 5px; height: 5px; background: var(--green); border-radius: 50%; box-shadow: 0 0 5px var(--green); animation: pulse-dot 2s infinite; }
        .topbar-divider { width: 1px; height: 16px; background: var(--border2); }
        .topbar-clock { font-size: .75rem; color: var(--text-muted); font-family: var(--font-mono); letter-spacing: .02em; }
        ...
        /* ── FLASH ───────────────────────────────────────────── */
        .flash { padding: 9px 13px; border-radius: var(--r-sm); margin-bottom: 16px; font-size: .84rem; display: flex; align-items: center; gap: 7px; font-weight: 500; }
        ...
        .page-header h1 { font-size: 1.2rem; font-weight: 600; color: var(--text-bright); line-height: 1.2; letter-spacing: -.03em; }
        .page-header .bc { font-size: .72rem; color: var(--text-muted); margin-top: 3px; letter-spacing: .02em; }

        /* ── STAT CARDS ──────────────────────────────────────── */
        .stat-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 12px; margin-bottom: 20px; }
        .stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); padding: 16px 16px 14px; position: relative; overflow: hidden; transition: border-color .15s; }
        ...
        .stat-card .sc-icon { width: 28px; height: 28px; border-radius: 5px; display: flex; align-items: center; justify-content: center; margin-bottom: 12px; }
        .stat-card .sc-icon svg { width: 14px; height: 14px; }
        ...
        .stat-card .sc-val { font-family: var(--font-mono); font-size: 1.75rem; font-weight: 500; line-height: 1; margin-bottom: 5px; letter-spacing: -.04em; }
        ...
        .stat-card .sc-label { font-size: .7rem; font-weight: 600; text-transform: uppercase; letter-spacing: .1em; color: var(--text-muted); }
        .stat-card .sc-sub   { font-size: .72rem; color: var(--text-dim); margin-top: 2px; }
        ...
        .card-header h2 { font-size: .84rem; font-weight: 600; color: var(--text-bright); display: flex; align-items: center; gap: 6px; letter-spacing: -.01em; }
        .card-header h2 svg { width: 14px; height: 14px; color: var(--text-muted); }
        ...
        thead th { padding: 9px 13px; text-align: left; font-size: .65rem; font-weight: 600; color: var(--text-dim); text-transform: uppercase; letter-spacing: .12em; border-bottom: 1px solid var(--border); white-space: nowrap; background: var(--surface2); }
        tbody tr { border-bottom: 1px solid rgba(23,32,48,.8); transition: background .08s; }
        tbody tr:hover { background: rgba(255,255,255,.014); }
        tbody tr:last-child { border-bottom: none; }
        tbody td { padding: 10px 13px; vertical-align: middle; }

        /* ── BADGES ──────────────────────────────────────────── */
        .badge { display: inline-flex; align-items: center; gap: 4px; padding: 2px 8px; border-radius: 4px; font-size: .65rem; font-weight: 600; text-transform: uppercase; letter-spacing: .07em; white-space: nowrap; font-family: var(--font-mono); }
        ...
        /* ── BUTTONS ─────────────────────────────────────────── */
        .btn { display: inline-flex; align-items: center; gap: 5px; padding: 7px 14px; border-radius: var(--r-sm); font-size: .82rem; font-weight: 600; font-family: var(--font); border: none; cursor: pointer; text-decoration: none; transition: all .12s; white-space: nowrap; letter-spacing: -.01em; }
        .btn svg { width: 14px; height: 14px; }
        .btn-primary { background: var(--accent); color: #fff; box-shadow: 0 1px 0 rgba(0,0,0,.2), 0 0 0 1px rgba(240,73,35,.5); }
        .btn-primary:hover { background: #d93e1c; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(240,73,35,.3), 0 0 0 1px rgba(240,73,35,.4); }
        .btn-secondary { background: var(--surface2); color: var(--text); border: 1px solid var(--border2); }
        .btn-secondary:hover { background: var(--surface3); }
        .btn-danger  { background: var(--red-dim);   color: var(--red);   border: 1px solid rgba(232,69,69,.2); }
        .btn-danger:hover  { background: rgba(232,69,69,.15); }
        .btn-success { background: var(--green-dim); color: var(--green); border: 1px solid rgba(30,201,109,.2); }
        .btn-success:hover { background: rgba(30,201,109,.14); }
        .btn-sm { padding: 5.5px 11px; font-size: .78rem; }
        .btn-xs { padding: 3px 8px;   font-size: .72rem; }

        /* ── FORMS ───────────────────────────────────────────── */
        .form-group { margin-bottom: 13px; }
        .form-group label { display: block; margin-bottom: 5px; font-size: .72rem; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: .09em; }
        .form-control { width: 100%; padding: 8px 10px; background: var(--surface2); border: 1px solid var(--border2); border-radius: var(--r-sm); color: var(--text); font-size: .85rem; font-family: var(--font); transition: border-color .12s, box-shadow .12s; }
        .form-control:focus { outline: none; border-color: var(--accent); box-shadow: 0 0 0 3px rgba(240,73,35,.08); }
        .form-control option { background: var(--surface2); }
        textarea.form-control { resize: vertical; min-height: 80px; }
        .form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 13px; }
        .form-check { display: flex; align-items: center; gap: 6px; font-size: .84rem; }
        ...
        .filter-bar .form-control { width: auto; padding: 6px 9px; }

        /* ── MODALS ──────────────────────────────────────────── */
        .modal-backdrop { display: none; position: fixed; inset: 0; background: rgba(0,0,0,.75); backdrop-filter: blur(6px); z-index: 200; align-items: center; justify-content: center; padding: 16px; }
        .modal-backdrop.open { display: flex; }
        .modal { background: var(--surface); border: 1px solid var(--border2); border-radius: var(--r-lg); width: 100%; max-width: 500px; max-height: 90vh; overflow-y: auto; box-shadow: 0 32px 80px rgba(0,0,0,.7), 0 0 0 1px rgba(255,255,255,.03); }
        .modal-header { padding: 15px 18px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
        .modal-header h3 { font-size: .9rem; font-weight: 600; color: var(--text-bright); letter-spacing: -.02em; }
        .modal-close { background: none; border: none; color: var(--text-muted); cursor: pointer; padding: 3px; border-radius: 4px; transition: color .1s, background .1s; }
        .modal-close svg { width: 16px; height: 16px; }
        .modal-close:hover { color: var(--text); background: var(--surface2); }
        .modal-body { padding: 18px; }
        .modal-footer { padding: 13px 18px; border-top: 1px solid var(--border); display: flex; gap: 7px; justify-content: flex-end; }
        ...
        /* ── ALERT ROWS ──────────────────────────────────────── */
        .alert-row { display: flex; align-items: flex-start; gap: 9px; padding: 10px 0; border-bottom: 1px solid rgba(23,32,48,.7); }
        ...
        .alert-title { font-weight: 500; font-size: .84rem; color: var(--text); letter-spacing: -.01em; }
        .alert-meta  { font-size: .72rem; color: var(--text-muted); margin-top: 2px; }

        /* ── INC ICONS ───────────────────────────────────────── */
        .inc-icon { width: 30px; height: 30px; border-radius: var(--r-sm); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .inc-icon svg { width: 14px; height: 14px; }
        ...
        .pagination a, .pagination span { padding: 5px 10px; border-radius: 4px; font-size: .76rem; text-decoration: none; font-family: var(--font-mono); }
        .pagination a { background: var(--surface2); border: 1px solid var(--border); color: var(--text-muted); transition: all .1s; }
        ...
        /* ── MISC ────────────────────────────────────────────── */
        .warning-strip { background: rgba(240,73,35,.06); border: 1px solid rgba(240,73,35,.15); border-radius: var(--r-sm); padding: 8px 12px; display: flex; align-items: center; gap: 7px; font-size: .82rem; color: var(--accent2); margin-bottom: 14px; }
        .warning-strip svg { width: 14px; height: 14px; flex-shrink: 0; }
        ...
        .detail-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px solid rgba(23,32,48,.6); font-size: .84rem; gap: 12px; }
        .detail-row:last-child { border-bottom: none; }
        .detail-row .dl { color: var(--text-muted); flex-shrink: 0; font-size: .76rem; text-transform: uppercase; letter-spacing: .06em; font-weight: 500; }
        .detail-row .dv { font-weight: 500; text-align: right; }
        .info-box { padding: 9px 12px; border-radius: var(--r-sm); font-size: .82rem; display: flex; align-items: flex-start; gap: 7px; margin-bottom: 14px; }
        ...
        /* ══════════════════════════════════════════════════════
           MOBILE RESPONSIVE — applies to ALL roles incl. citizen
           ══════════════════════════════════════════════════════ */
        @media (max-width: 768px) {
            ...
            .sb-nav a { padding: 11px 11px 11px 14px; }
            ...
            tbody td .btn-xs { padding: 5px 10px; font-size: .74rem; }
        }

        @media (max-width: 480px) {
            .stat-grid { grid-template-columns: 1fr 1fr !important; }
            .topbar-status span { display: none; }
            .topbar-status { padding: 4px 7px; }
            html { font-size: 14px; }
        }
    </style>
    @stack('styles')
