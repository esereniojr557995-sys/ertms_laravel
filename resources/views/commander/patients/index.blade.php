...
<div class="page-header">
    <div><h1>Patients</h1><div class="bc">Commander / Medical</div></div>
...
<div class="card">
    <div class="table-wrap">
        <table>
            <thead>
                <tr><th>Patient</th><th>Triage</th><th>Incident</th><th>Status</th><th>Hospital</th><th>Actions</th></tr>
            </thead>
            <tbody>
            @forelse($patients as $p)
            @php
                $isLocked = in_array($p->status, ['discharged', 'deceased']);
            ...
            @endphp
            <tr style="{{ $isLocked ? 'opacity:.6' : '' }}">
                <td>
                    <div style="font-weight:600;display:flex;align-items:center;gap:7px">
                        {{ $p->name }}
                        @if($isLocked)
                        <span style="display:inline-flex;align-items:center;gap:3px;font-size:.62rem;font-weight:600;color:{{ $statusColor }};background:{{ $p->status==='discharged' ? 'var(--green-dim)' : 'rgba(45,66,87,.2)' }};border:1px solid {{ $p->status==='discharged' ? 'rgba(30,201,109,.2)' : 'var(--border)' }};padding:1px 6px;border-radius:10px;letter-spacing:.04em">
                            <i data-lucide="lock" style="width:9px;height:9px"></i>
                            {{ strtoupper($p->status) }}
                        </span>
                        @endif
                    </div>
                    <div style="font-size:.72rem;color:var(--text-muted)">{{ $p->age ?? '?' }} yrs · {{ $p->gender }}</div>
                </td>
                ...
                <td>
                    <span class="badge badge-{{
                        $p->status === 'admitted'    ? 'completed'  :
                        ($p->status === 'discharged' ? 'completed'  :
                        ($p->status === 'deceased'   ? 'cancelled'  :
                        ($p->status === 'on_scene'   ? 'active'     : 'in_progress')))
                    }}">{{ str_replace('_',' ', $p->status) }}</span>
                </td>
                ...
                <td>
                    @if($isLocked)
                        <span style="font-size:.74rem;color:var(--text-dim);display:flex;align-items:center;gap:4px">
                            <i data-lucide="{{ $p->status === 'discharged' ? 'check-circle' : 'x-circle' }}"
                               style="width:13px;height:13px;color:{{ $statusColor }}"></i>
                            {{ ucfirst($p->status) }}
                        </span>
                    @else
                        <form method="POST" action="{{ route('commander.patients.update', $p) }}"
                              style="display:flex;gap:4px;align-items:center">
                            @csrf @method('PUT')
                            <select name="status" class="form-control"
                                    style="width:auto;padding:4px 6px;font-size:.73rem">
